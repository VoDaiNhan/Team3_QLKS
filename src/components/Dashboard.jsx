  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      ...
      try {
        ...
        // 1. Lấy tổng đặt phòng từ API mới
        try {
          const datPhongRes = await apiFetch('https://qlks-0dvh.onrender.com/api/DatPhong?pageNumber=1&pageSize=10');
          const datPhongData = await datPhongRes.json();
          // Giả sử API trả về dạng { data: { totalItems: ... } } hoặc { totalItems: ... }
          totalBookings = datPhongData.data?.totalItems ?? datPhongData.totalItems ?? 0;
        } catch (e) {
          totalBookings = 0;
        }

        // 2. Lấy doanh thu ngày hiện tại
        try {
          const revenueRes = await apiFetch(`https://qlks-0dvh.onrender.com/api/ThongKe/ngay?ngay=${todayStr}`);
          if (revenueRes.ok) {
        ...
        try {
          ...
          availableRooms = roomStatusData.data?.['Trống'] || 0;
          occupiedRooms = roomStatusData.data?.['Đang sử dụng'] || 0;
        } catch (e) {
          availableRooms = 0;
          occupiedRooms = 0;
        }
